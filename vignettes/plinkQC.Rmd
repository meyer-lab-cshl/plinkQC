...
running the provided pre-trained classier are also needed from https://github.com/meyer-lab-cshl/plinkQC. [Git Large File Storage extention](https://git-lfs.com/) needs to be pre-installed to get the correct files.
...
to correctly format genomic data that is in the hg38 annotation. The name of the
files (before the .acount or .eigenvec.allele) for the loading matrices must be included in the path2load_mat. 
...
After correctly formatting the data, we can use the provided function to return the 
predicted ancestry. Because this example uses only a small subset of variants for demonstration, additional noise is expected.
