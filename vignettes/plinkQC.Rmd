...
functions:
    ...
    (iv) `superpop_classfication`: for prediction of genomic ancestry
...
The ancestry portion of the package requires that the data should be in PLINK 2.0 format (i.e pgen, pvar, psam files), 
and the variants identifiers should be formatted similar to the following example: 1:12345[hg38]. `perIndividualQC` will run the functions `convert_to_plink2()` and `rename_variant_identifiers()` to convert the data
...
For ancestry identification, there are 597 SNPs that overlap between the study and  reference data, roughly 0.2% of the reference dataset. Thus, additional noise is expected and visible in the plot below.
