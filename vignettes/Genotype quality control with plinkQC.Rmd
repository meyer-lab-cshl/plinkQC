...
output: 
    pdf_document:
        ...
        toc_depth: 2
vignette: >
  %\VignetteIndexEntry{Genotype quality control with plinkQC}
...
qcdir <- "/Users/hannah/RPackagesDevel/plinkQC/inst/extdata"
...
fail_individuals <- perSampleQC(qcdir=qcdir, alg=alg,
                                #refSamplesFile=refSamplesFile, 
                                #refColorsFile=refColorsFile,
                                externalSexID="Bru.Number",
                                do.check_ancestry=FALSE,
                                prefixMergedDataset="gencall.sanger12.HapMapIII.pruned.pca",
                                interactive=TRUE, verbose=TRUE)

```

```{r overview sample QC}
overview <- overviewPerSampleQC(fail_individuals, interactive=TRUE)
```
