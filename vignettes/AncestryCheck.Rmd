...
awk 'BEGIN {OFS="\t"} FNR==NR {a[$1$4]=$5$6; next} \
    ($1$4 in a && a[$1$4] != $5$6 && a[$1$4] != $6$5)  {print $2}' \
    $qcdir/$name.pruned.bim $qcdir/$refname.pruned.bim > \
...
### Filter study data for the same SNPs as in reference data
We will use the final list of corrected variants from the reference study to match the snps in our study:

```{bash filter, eval=FALSE}
cut -f 2 $qcdir/$refname.clean.bim > $qcdir/$refname.clean.keepSNPs

plink --bfile  $qcdir/$name.pruned \
      --extract $qcdir/$refname.clean.keepSNPs \
      --make-bed \
      --allow-extra-chr 0 \
      --out $qcdir/$name.pruned.clean
mv  $qcdir/$name.pruned.clean.log $qcdir/plink_log/$name.pruned.clean.log
```
...
plink --bfile $qcdir/$name.pruned.clean  \
...
performed PCA on the combined study and reference datasets  and identified 
...
train a random forest classifier that uses up to 20 PCs to sufficiently identify
individuals of all ancestries in the reference dataset. 

We use the .eigenvec file that was created when preparing files for ancestry 
estimation, and read in its dataframe. The file contains the eigenvectors for the PCA on the LD-prunced genotype data.  The .eigenvec file only included the eigenvalues per PC for each individual and did not include any information regarding individual's ancestries. Therefore, I had to acquire the file available in the plinkQC download, which contains the individual's ID, their family ID, and their associated ancestry and read in that dataframe, as obtained from the 1000 Genomes database. 

Parameters portion 
It is important to tune a few parameters to see which produces the best forest. 
