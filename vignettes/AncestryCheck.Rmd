...
We can determine the ancestry of samples using the provided already trained random 
forest classification algorithm. This was trained on the princple components of 
cleaned genomics data from the 1000 genomes dataset. To do so, we will first 
convert the genomic data from PLINK 1.9 format to PLINK 2.0 format if needed. Then, 
we can predict the ancestry using the trained random forest.

```
convert_to_plink2( PUT IN INSIDES HERE)
ancestries <- superpop_classification( PUT IN SAME INSIDES)
```
...
we recommend training your own random forest. To do so, we will first need to 
download the 1000 genomes dataset which is described in detail in [Processing 1000 Genomes reference data for ancestry estimation](Genomes1000.Rmd): .
...
Then, we will remove variants with a low minor allele count and variants within 
linkage disequilibrium with each other. 
NEED TO RUN AND SEE IF A) EVERYTHING WORKS PROPERLY AND IF A LOG FILE IS CREATED
AND SEE HOW THEY PREVIOUSLY CALLED THE NAME OF THE FILE FOR LDs

```{bash low MAF and prune, eval=FALSE}
plink2 --pfile filtered_hg38\
       --maf \
       --ac-founders \
       --make-pgen \
       --out filtered_hg38.maf

plink2 --pfile filtered_hg38.maf\
       --exclude $LINKAGEFILE FILE IN HERE \
       --indep-pairwise 50 5 0.2 \
       --out filtered_hg38.renamed

plink2 --pfile filtered_hg38.renamed \
       --extract range filtered_hg38.renamed.prune.in \
       --make-pgen \
       --out filtered_hg38.pruned
```

After filtering out the low minor allele frequency and pruning, we will extract the principle components from the data.
I THINK I SHOULD ACTUALLY BE INCLUDING FOUNDERS IN THIS SO MAKE SURE TO RERUN THIS CODE AND TAKE OUT THAT ONE LINE
``` {bash pca, eval=FALSE}
...
After running a PCA, we will proejct the original dataset onto the principle components.

From this, this will be used to train the random forest algorithm in R. 


run a pca
then the R code for how that becomes a RF
then carolines fine tuning information 

To predict data, you need to project the newdata onto the principle components. 
This is done with the --score function in plink2
# References
