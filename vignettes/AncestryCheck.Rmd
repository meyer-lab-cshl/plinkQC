...
[Plink v2](https://www.cog-genomics.org/plink/2.0/) is needed for this portion of the package. 
...
Before use, the study data should be in the new hg38 annotation. USCSC's [liftOver](https://genome.ucsc.edu/cgi-bin/hgLiftOver) tool
...
rename_variant_identifiers(indir=indir, qcdir=qcdir, name=name, 
                           path2plink2 = path2plink2)
name <- paste0(name, ".renamed")
ancestries <- superpop_classification(indir=indir, qcdir=qcdir, name=name, 
                                      path2plink2 = path2plink2) 
head(ancestries)
```
...
we recommend training your own random forest. To do so, we will first need to set up the needed bash variables and download the 1000 genomes dataset into qcdir which is described in 
...
plink2 --pfile filtered_hg38.renamed\
       --exclude range $LINKAGEFILE FILE IN HERE \
...
NEED TO FIX FILE PATH AND PUT FILES IN EXTDATA FOLDER
```{r, eval = FALSE}
filepath <- "~/Documents/meyerlab/plinkQC_validation/eigenvectors/founders_inc/all_hg38.projection.sscore"
...
```{r, eval = FALSE}
proj <- read.csv(
  file="~/Documents/meyerlab/plinkQC_validation/eigenvectors/founders_inc/all_hg38.projection.sscore", 
  sep='\t', header = TRUE)
ancestry_info <- 
  read_delim("~/Documents/meyerlab/Aim1/Ancestry/build_forest/Genomes1000_ID2Pop.txt")
superpop <- 
  read_csv("~/Documents/meyerlab/Aim1/Ancestry/build_forest/AncestryGeoLocations.csv")
...
Then, we divide an even number from each ancestral group to be used in the testing data
``` {r, eval = FALSE}
n_individuals <- 20
set.seed(123)
