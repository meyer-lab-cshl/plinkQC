...
This package contains a function to predict ancestry of samples. We trained a random forest algorithm on the principal components of
cleaned genomics data from the [harmonized 1000 Genomes and Human Genome Diversity Project dataset](https://genome.cshlp.org/content/34/5/796).
...
the package. Additional loading matrices are needed for the PCA projection used in the model.
...
We will set up a temporary directory for qcdir and name the variables. The name of the files (before the .acount or .eigenvec.allele) for the loading matrices must be included in the `path2load_mat` variable. 
...
to use the tool can be found on the processing [HapMap III reference data vignette](HapMap.html).
...
After correctly formatting the variants, we can use the provided function to return the 
predicted ancestry. Because the example dataset has only a small number of variants 
overlapping with the reference dataset, additional noise is expected.
...
detail in [Processing 1000 Genomes reference data for ancestry estimation](Genomes1000.html). We will use a test study dataset included in the package as an example for the workflow.  
...
```
After here, is where you need to remove letters from ids.
...
we will rename the variant identifiers to a common scheme based on chromosome number and basepair location by using plinkQC's rename variant identifier function.
...
After filtering for SNPS shared in the study, we will conduct marker and sample quality
control on the reference dataset. This is to ensure only robust data is used as 
a reference. To reduce correlations between SNPs, we will prune for variants in linkage disequilibrium (LD) after removing low quality markers. 
This is done with plinkQC's pruning_ld() function. As part of the function, ranges
of known high-LD are excluded. These ranges are originally provided by @Anderson2010. 
...
### PCA
After conducting quality control, we will extract the principal
components from the data to reduce the input space. 
...
After running a PCA, we will project the original dataset onto the principal components to rescale. This ensures that future projections will be in the same scale.
