...
Note: If you are using 1000Genomes reference data, the refname will be 
'all_hg38'.  
...
```{bash setup, eval=FALSE}
awk '{print "chr" $1, $4 -1, $4, $2 }' $qcdir/data.no_ac_gt_snps.bim | \      
    sed 's/chr23/chrX/' | sed 's/chr24/chrY/' > \                
    $qcdir/data.no_ac_gt_snps.tolift
```
...
```{bash setup, eval=FALSE}
liftOver $qcdir/data.no_ac_gt_snps.tolift \
    /Users/meyerlab/projects/ancestry/liftOver/hg19ToHg38.over.chain.gz \ 
    $qcdir/data.no_ac_gt_snps.hg38 $qcdir/data.no_ac_gt_snps.unMapped  
```
...
awk '{print $4}' $qcdir/data.no_ac_gt_snps.hg38 > \
    $qcdir/data.no_ac_gt_snps.hg38.snps   
# extract updated positions
awk '{print $4, $3}' $qcdir/data.no_ac_gt_snps.hg38 > \ 
    $qcdir/data.no_ac_gt_snps.hg38.pos
```
...
```{bash setup, eval=FALSE}
df <- read.table(
    "/Users/meyerlab/qcdir/data.no_ac_gt_snps.hg38.bim",
    header = FALSE,
    sep = "\t",
    dec = "."
  )
...
write.table(
  df,
  file = "/Users/meyerlab/qcdir/data.no_ac_gt_snps.hg38.bim",
  sep = "\t",
  quote = FALSE,
  row.names = FALSE,
  col.names = FALSE
