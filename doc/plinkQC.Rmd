...
## Predicting ancestry of human samples

The identification of ancestry is done by training the classifier on the
principal components of a reference dataset. The reference dataset is composed of 
individuals a harmonized dataset of the 1000 genomes individuals with 
the Human Genome Diversity Project. A pre-trained classifier is included
within the package to predict the ancestry. `superpop_classification()` returns a plot
of the predicted probabilities of each ancestry, the dataframe containing
the predicted probability, and a dataframe with the majority ancestry for each individual. 

There are additional functions to rename the variant in the correct format and put the data in plink2 format if needed for the classifier to work properly.
```{r check ancestry, eval=FALSE, fig.height=3, fig.width=5, fig.align='center'}

superpop_classification(indir = indir, 
                        name = name, qcdir = qcdir, 
                        path2plink2 = path2plink2, path2load_mat = "../plinkQC_validation/ancestry_unrel_model/merged_chrs.postQC.train.pca")
#function for superpop classification 
